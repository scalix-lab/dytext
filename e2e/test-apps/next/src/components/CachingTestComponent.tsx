...
export default function CachingTestComponent() {
  ...
  if (isLoading) return <div data-testid="cache-loading">Testing caching behavior...</div>;
  if (error) return <div data-testid="cache-error">Error: {error}</div>;
  if (!cacheResults) return <div data-testid="cache-no-data">No cache test data available</div>;

  const isCached = JSON.stringify(cacheResults.firstCall) === JSON.stringify(cacheResults.secondCall);
    ...
    <div data-testid="caching-test-component">
      ...
      <div data-testid="cache-comparison">
        <div data-testid="first-call-result">
          First call result: {JSON.stringify(cacheResults.firstCall).substring(0, 100)}...
        </div>
        <div data-testid="second-call-result">
          Second call result: {JSON.stringify(cacheResults.secondCall).substring(0, 100)}...
        </div>
      ...
      <div data-testid="performance-metrics">
        <div data-testid="first-call-time">First call time: {cacheResults.callTimes.first}ms</div>
        <div data-testid="second-call-time">Second call time: {cacheResults.callTimes.second}ms</div>
        <div data-testid="third-call-time">Third call time: {cacheResults.callTimes.third}ms</div>
      </div>
      
      <div data-testid="cache-status">
        <div data-testid="is-cached">Results are identical: {isCached ? 'Yes' : 'No'}</div>
        <div data-testid="is-faster">Second call is faster: {isFaster ? 'Yes' : 'No'}</div>
      </div>
    </div>
  );
}
