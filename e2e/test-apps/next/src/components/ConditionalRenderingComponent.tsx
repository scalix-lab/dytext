...
export default function ConditionalRenderingComponent() {
  ...
  if (isLoading) return <div data-testid="conditional-loading">Loading conditional data...</div>;
  if (error) return <div data-testid="conditional-error">Error: {error}</div>;

  // Conditional rendering based on data availability
  const hasUserProfile = userProfile && userProfile.fields && userProfile.fields.length > 0;
  const hasProductCatalog = productCatalog && productCatalog.fields && productCatalog.fields.length > 0;
      ...
      {hasBothModels && (
        <div data-testid="combined-section">
          ...
          <div data-testid="total-fields">
            Total Fields: {userProfile.fields.length + productCatalog.fields.length}
          </div>
        </div>
      )}
