...
export default function FormMappingComponent() {
  ...
  if (isLoading)
    return <div data-testid="form-loading">Loading form data...</div>;
  if (error) return <div data-testid="form-error">Error: {error}</div>;
  if (!formData)
    return <div data-testid="form-no-data">No form data available</div>;
        ...
        {formData.fields.map((field, index) => {
          ...
          return (
            <div key={index} data-testid={`form-field-${index}`}>
              <label htmlFor={fieldName} data-testid={`field-label-${index}`}>
                {fieldName}
              </label>
              <input
                id={fieldName}
