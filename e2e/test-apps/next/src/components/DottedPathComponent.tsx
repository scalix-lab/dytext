...
export default function DottedPathComponent() {
  ...
  if (isLoading)
    return <div data-testid="dotted-path-loading">Loading dotted paths...</div>;
  if (error) return <div data-testid="dotted-path-error">Error: {error}</div>;
  if (!dottedPathData)
    return (
      <div data-testid="dotted-path-no-data">No dotted path data available</div>
    );
  ...
  return (
    <div data-testid="dotted-path-component">
      ...
      <div data-testid="first-field-description">
        {dottedPathData.firstFieldDescription || "No description"}
      </div>
      <div data-testid="user-profile-name">
        {dottedPathData.userProfileName}
      </div>
      <div data-testid="non-existent-path">
        {dottedPathData.nonExistentPath === undefined ? "undefined" : "exists"}
      </div>
    </div>
  );
}
