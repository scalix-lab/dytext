...
export default function RealTimeDataComponent() {
  ...
  if (isLoading && refreshCount === 0) return <div data-testid="realtime-loading">Loading real-time data...</div>;
  if (error) return <div data-testid="realtime-error">Error: {error}</div>;
  if (!data) return <div data-testid="realtime-no-data">No real-time data available</div>;
