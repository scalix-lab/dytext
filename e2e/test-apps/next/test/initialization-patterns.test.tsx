...
function EnvVarComponent() {
  ...
  if (isLoading) return <div data-testid="env-loading">Loading with env var...</div>;
  ...
  return (
    <div data-testid="env-component">
      ...
      <div data-testid="env-models-count">Models: {Object.keys(data).length}</div>
...
function ProperEnvVarComponent() {
  ...
  if (isLoading) return <div data-testid="proper-env-loading">Loading with proper env var...</div>;
  if (error) return <div data-testid="proper-env-error">Error: {error}</div>;
  if (!data) return <div data-testid="proper-env-no-data">No data available</div>;

  return (
    <div data-testid="proper-env-component">
      <h2 data-testid="proper-env-title">Proper Environment Variable Initialization</h2>
      <div data-testid="proper-env-models-count">Models: {Object.keys(data).length}</div>
      <div data-testid="proper-env-models-list">{Object.keys(data).join(', ')}</div>
    </div>
  );
}
...
function ManualInitComponent() {
  ...
  if (isLoading) return <div data-testid="manual-loading">Loading with manual init...</div>;
  ...
  return (
    <div data-testid="manual-component">
      ...
      <div data-testid="manual-models-count">Models: {Object.keys(data).length}</div>
...
function NoInitComponent() {
  ...
  if (isLoading) return <div data-testid="no-init-loading">Loading without init...</div>;
  ...
  return (
    <div data-testid="no-init-component">
      ...
      <div data-testid="no-init-models-count">Models: {Object.keys(data).length}</div>
    </div>
  );
}
...
describe('DyText Initialization Patterns', () => {
  ...
  describe('Initialization Requirements', () => {
    it('should fail without explicit initialization even with environment variable set', async () => {
      ...
      expect(screen.getByTestId('env-error').textContent).toContain('initialized');
    });
    ...
    it('should work with explicit token initialization', async () => {
      ...
      expect(screen.getByTestId('proper-env-title')).toHaveTextContent('Proper Environment Variable Initialization');
      ...
      const modelsList = screen.getByTestId('proper-env-models-list').textContent;
    ...
    it('should fail without environment variable or manual initialization', async () => {
      ...
      expect(screen.getByTestId('no-init-error').textContent).toContain('initialized');
    });
  });
  ...
  describe('Manual Initialization', () => {
    it('should work with manual initialization without environment variable', async () => {
      ...
      expect(screen.getByTestId('manual-title')).toHaveTextContent('Manual Initialization');
