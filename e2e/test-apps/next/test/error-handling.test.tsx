...
describe("DyText Error Handling", () => {
  ...
  it("should handle non-existent models gracefully", async () => {
    ...
    expect(screen.getByTestId("non-existent-model-result")).toHaveTextContent(
      "undefined",
    );

    // Verify invalid paths return undefined
    expect(screen.getByTestId("invalid-path-result")).toHaveTextContent(
      "undefined",
    );
    expect(screen.getByTestId("deep-invalid-path-result")).toHaveTextContent(
      "undefined",
    );

    // Verify empty path still returns data
    expect(screen.getByTestId("empty-path-result")).toHaveTextContent(
      "has data",
    );
  });
